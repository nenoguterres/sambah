...
const DEFAULT_FILES = {
  signals: "pay-perola-signals.json",
  suggestions: "pay-perola-suggestions.json"
};

export const PAY_PEROLA_SIGNAL_CREATED = "pay.perola.signal.created";

export class PayPerolaBridgeService {
  constructor({ dataDir = defaultDataDir, eventBus = null, workhubService = null, now = () => new Date(), idGenerator = () => crypto.randomUUID() } = {}) {
  ...
  async registrarSinalPay(payload = {}) {
    const signal = await this.#append(this.signalsFile, {
      id: this.idGenerator(),
      origem: "pay",
  ...
  async registrarSugestaoPerola(payload = {}) {
    ...
    await registerWorkhubTask(this.workhubService, "pay", suggestion);
    return suggestion;
  }
